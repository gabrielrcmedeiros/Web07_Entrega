...
export class Vitrine {
  ...
  adicionarCesta(obj: Produto) {
    /* Recupera os dados do armazenamento local ou inicializa um array vazio */
    let cestaStorage = localStorage.getItem('cesta');
    let itens = cestaStorage ? JSON.parse(cestaStorage) : [];

    /* Verifica se o produto selecionado já encontra-se na lista da cesta */
    let itemExistente = itens.find((item: any) => item.produto.codigo === obj.codigo);

    /* Determina o valor a ser aplicado (prioriza o valor promocional, se aplicável) */
    let precoEfetivo = obj.promo > 0 ? obj.promo : obj.valor;

    if (itemExistente) {
      /* Incrementa a quantidade e atualiza o custo total do item específico */
      itemExistente.quantidade += 1;
      itemExistente.valorTotal = itemExistente.quantidade * precoEfetivo;
    } else {
      /* Insere um novo registro na lista de compras */
      itens.push({
        produto: obj,
        quantidade: 1,
        valorTotal: precoEfetivo
      });
    }

    /* Atualiza o armazenamento local com a nova estrutura da lista */
    localStorage.setItem('cesta', JSON.stringify(itens));
    alert('Produto adicionado à cesta de compras!');
  }
}